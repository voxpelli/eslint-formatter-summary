 ...
 * per-file synthetic rule name (unused-disable / missing-rule). `line` is `?`
 * when the message emitted no line (fatal parse errors can omit it).
 *
 ...
 * @property {string} [eslintVersion]               ESLint version label stamped by `prepare --eslint-version` (absent when not supplied).
 ...
 * @property {string[]} [syntheticKeys]          Parenthesized synthetic bucket IDs present in this run (absent when no synthetic messages were classified).
 ...
 * escape `baseDir` keep their `../` prefix; the downstream renderer
 * (`renderFileSpan`) rejects `..` URL segments and falls back to plain text.
 *
 * @param {unknown} raw         Parsed JSON output of `eslint --format json`.
 * @param {{ project?: string, eslintVersion?: string, baseDir: string }} options
 * @returns {ProjectResult | undefined}
 */
export function prepareProjectResult (raw, { baseDir, eslintVersion = '', project = '' }) {
  ...
  return errorCount === 0 && warningCount === 0
    ? undefined
    : {
        // Omit the field when empty so the shape stays additive for existing
        // consumers (sibling artifacts without eslintVersion remain valid).
...
function prepareLintMessage (rawMsg) {
  if (
    ...
    // `line` is optional — fatal parse errors can emit no line (or null).
    // Messages without one are still bucketed, with a `?` placeholder in the
    // file entry (matching the sibling tool's `msg.line ?? '?'` convention).
    !isOptionalKeyWithType(rawMsg, 'line', ['number', 'null'])
  ) {
    return;
  }
  ...
  // `line` is optional at runtime (absent or null), so the literal is not a
  // full LintMessage — the guards above narrow the required fields, and the
  // `??` fallbacks below fill the gaps for classifyMessage.
  const msg = {
    ...rawMsg,
