...
/**
 * Render a project label with an optional `(eslint <version>)` suffix. The
 * version is artifact-sourced (untrusted), so it is sanitized and HTML-escaped
 * before interpolation — a tampered `eslintVersion` must not inject markup
 * into the `<summary>` line or the tail-summary table.
 *
 * @param {ProjectResult} result
 * @returns {string}
 */
export const renderProjectLabelWithVersion = (result) =>
  renderProjectLabel(result.project) +
  (typeof result.eslintVersion === 'string' && result.eslintVersion
    ? ` (eslint ${escapeHtml(sanitizeUntrusted(result.eslintVersion))})`
    : '');
 ...
 * synthetic-key per-file rule name, and `line` is `?` when the message
 * emitted none (fatal parse errors). When the slug is valid we emit a GitHub
 ...
 * `<sub>` tag.
 *
 * @see renderFileEntry in format-results.js — formatter twin; path-only (no line anchor), wrapped in `<li>`.
...
export function renderFileSpan (entry, slug) {
  ...
  const safePath = filePath !== undefined && !filePath.split('/').includes('..') && filePath.isWellFormed();
