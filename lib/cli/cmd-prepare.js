...
const run = async (argv, _importMeta, { parentName }) => {
  const cli = peowly({
    ...
    examples: [
      ...
      '--project voxpelli/ref-calc project/eslint-results.json',
    ...
    options: {
      ...
      cwd: {
        type: 'string',
        'default': '',
        description: 'Strip-prefix for relative paths (default: directory of input file)',
      },
    },
  });
  ...
  if (input.length !== 1) {
    process.stderr.write('eslint-summary prepare: expected exactly one <input-file> positional\n');
    process.exit(2);
  }
  const inputPath = /** @type {string} */ (input[0]);
  ...
  try {
    ...
    const msg = err instanceof Error ? err.message : String(err);
    process.stderr.write(`eslint-summary prepare: invalid JSON in ${inputPath}: ${msg}\n`);
    process.exit(1);
  }

  const projectName = flags.project || process.env['EFS_PROJECT_NAME'] || '';
  const cwd = flags.cwd || path.resolve(path.dirname(inputPath));

  const result = prepareProjectResult(parsed, { cwd, project: projectName });
