import { isC0C1Control, stripControls } from './utils/text.js';
 ...
 *
 * @see {@link escapeHtml} composes with sanitize at every render sink — scrubbed output is then HTML-escaped as normal.
...
export function sanitizeUntrusted (text, { maxLength = DEFAULT_MAX_LENGTH } = {}) {
  ...
  for (const ch of stripControls(text)) {
    ...
    const isControl = code !== undefined && isC0C1Control(code);
