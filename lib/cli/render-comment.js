 ...
 * @returns {string}
 */
export function renderSuccess (projectCount) {
...
export function renderComment (results, { fileCap } = {}) {
  ...
  // First non-zero clause is introduced with an em-dash; subsequent ones
  // chain with a comma. A warnings-only fleet (every project at `warn`
  // severity) must not emit `**…**, N warnings` with a dangling comma.
  let sep = ' — ';
  if (errors > 0) {
    md += `${sep}${errors} errors (${fixableErrors} fixable)`;
    sep = ', ';
  }
  if (warnings > 0) md += `${sep}${warnings} warnings (${fixableWarnings} fixable)`;
  md += '\n\n';
