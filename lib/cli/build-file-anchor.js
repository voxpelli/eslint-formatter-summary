...
export function renderFileSpan (entry, slug) {
  ...
  // Split on the trailing `:line` BEFORE `sanitize` runs, so the cosmetic
  // length cap cannot chop the line suffix away and silently demote the
  // anchor to an unlinked `<code>` span for paths near PATH_MAX.
  ...
  if (rawMatch) {
    ...
    pathAndLine = sanitize(rawPath, { maxLength: Math.max(1, PATH_MAX - lineSuffix.length) }) + lineSuffix;
  } else {
    pathAndLine = sanitize(rawPathAndLine, { maxLength: PATH_MAX });
  }
