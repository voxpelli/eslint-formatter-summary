 ...
 * @returns {string}
 */
const padNum = (n, len) => n.toString().padStart(len);
...
/**
 * Max rendered length of `rules[*][prop]`. Leading `0` is load-bearing for
 * the empty-array case (no rules → zero column width).
 *
 * @param {'errors' | 'warnings' | 'fixable' | 'id'} prop
 * @param {MessageSummary[]} rules
 * @returns {number}
 */
const maxPropLen = (prop, rules) =>
  Math.max(0, ...rules.map((r) => r[prop].toString().length));

/** @type {CountingHelper} */
const maxErrorLen = (rules) => maxPropLen('errors', rules);
/** @type {CountingHelper} */
const maxWarningLen = (rules) => maxPropLen('warnings', rules);
/** @type {CountingHelper} */
const maxFixableLen = (rules) => maxPropLen('fixable', rules);
/** @type {CountingHelper} */
const maxRuleLen = (rules) => maxPropLen('id', rules);

/**
 * @param {'errors' | 'warnings' | 'fixable'} prop
 * @param {MessageSummary[]} rules
 * @returns {number}
 */
const sumProp = (prop, rules) => rules.reduce((n, r) => n + r[prop], 0);

/** @type {CountingHelper} */
const totalErrors = (rules) => sumProp('errors', rules);
/** @type {CountingHelper} */
const totalWarnings = (rules) => sumProp('warnings', rules);
/** @type {CountingHelper} */
const totalFixable = (rules) => sumProp('fixable', rules);
/** @type {CountingHelper} */
...
const markdownLink = (text, url) => {
  ...
  // Escape attribute-breaking chars (`"` `<` `>`) and Markdown-structure
  // chars (`)` breaks link syntax, `|` breaks GFM table cells) defensively
  // even though the scheme allowlist already rejects the most dangerous protocols.
  const safeForAttr = safe.replaceAll('"', '%22').replaceAll('<', '%3C').replaceAll('>', '%3E').replaceAll(')', '%29').replaceAll('|', '%7C');
 ...
 *
 * @see {@link renderFileSpan} — CLI twin that additionally emits a GitHub `blob/HEAD/<path>#L<line>` anchor.
 * @param {string} entry
 * @returns {string}
 */
const renderFileEntry = (entry) => {
  const tab = entry.indexOf('\t');
  if (tab === -1) return `<li>${escapeHtml(sanitizeUntrusted(entry, { maxLength: PATH_MAX }))}</li>`;
  const path = sanitizeUntrusted(entry.slice(0, tab), { maxLength: PATH_MAX });
  const detail = sanitizeUntrusted(entry.slice(tab + 1));
...
/**
 * @param {MessageSummary[]} rules
 * @param {{ markdown: boolean, rulesMeta: ESLint.LintResultData["rulesMeta"], fileCap?: number | undefined }} options
 * @returns {string}
 */
const constructSummary = (rules, { fileCap, markdown, rulesMeta }) => {
  ...
  const cell = markdown
    ? (/** @type {number} */ n, /** @type {number} */ len) => (n === 0 ? '-' : n.toString()).padStart(len)
    : padNum;
  ...
  return rules
    .map((rule) => {
      ...
      return markdown
        ? `| ${errors} | ${warnings} | ${fixable} | <details><summary>${markdownLink(escapeHtml(sanitizeUntrusted(id)), docsUrl).padEnd(maxRule)}</summary>${filesList}</details> |`
        : `${
