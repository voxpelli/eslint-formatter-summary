...
import { renderProjectLabelWithVersion } from './build-file-anchor.js';
import { toCount } from './coerce.js';
import { PROJECT_BLOCK_CLOSE, PROJECT_BLOCK_OPEN_TAG } from './render-project-block.js';
...
const renderTailSummary = (tail) => {
  ...
  const rows = tail.map((r) => {
    // Coerce count fields defensively via toCount — validated by
    // isValidProjectResult upstream; toCount is defense-in-depth for
    // direct/test callers that bypass the validator.
    ...
    return `| ${renderProjectLabelWithVersion(r)} | ${errors} | ${warnings} | ${fixStr} |`;
  }).join('\n');
...
export function truncateComment (md, results, { sizeCap = DEFAULT_SIZE_CAP } = {}) {
  ...
  const closeAnchor = PROJECT_BLOCK_CLOSE;
  const lastClose = slice.lastIndexOf(closeAnchor);
  // When the slice contains no close anchor (first project block alone
  // exceeds the `sizeCap - HEADROOM` window), keep the rendered header /
  // footnote prefix that precedes the first project block and roll every
  // project into the tail summary. Falling back to `slice` would leave an
  // unclosed `<details>` whose implicit close would be absorbed by the tail
  // summary's `</details>`, visually nesting the tail inside the partial
  // first-project block. When even the prefix is absent (slice starts
  // mid-block or is empty), emit an empty kept section.
  //
  // Assumption: the first `\n<details>` in the slice is the first project
  // block — the rendered header and footnote (`<sub><em>…</em></sub>`) never
  // contain `<details>`, and project blocks always start with `<details>\n`.
  const firstBlockStart = slice.indexOf('\n' + PROJECT_BLOCK_OPEN_TAG);
  let keptMd;
  if (lastClose !== -1) {
    keptMd = slice.slice(0, lastClose + closeAnchor.length);
  } else if (firstBlockStart !== -1) {
    keptMd = slice.slice(0, firstBlockStart + 1);
  } else {
    keptMd = '';
  }
  const keptCount = keptMd.split(closeAnchor).length - 1;
  ...
  // fits under (sizeCap - HEADROOM) so keptMd + a short fallback is safe for
  // realistic caps.
  ...
  const fallbackAssembled = keptMd + fallback;
  if (Buffer.byteLength(fallbackAssembled, 'utf8') <= sizeCap) return fallbackAssembled;
  // Even the fallback doesn't fit (pathological sub-~100-byte caps) — emit a
  // minimal marker, else nothing. Both branches keep the output within
  // `sizeCap`, mirroring the formatter truncator's degradation.
  const minimal = '\n_(truncated)_\n';
