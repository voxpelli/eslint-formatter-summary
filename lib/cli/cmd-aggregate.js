...
const run = async (argv, _importMeta, { parentName }) => {
  ...
  const caller = 'eslint-summary aggregate';
  const projectCount = parseNumericFlag(flags['project-count'] || env['EXTERNAL_PROJECT_COUNT'] || '', '--project-count', caller);
  const sizeCap = parseNumericFlag(flags['size-cap'] || env['EFS_SIZE_CAP'] || '', '--size-cap', caller);
  const fileCap = parseNumericFlag(flags['file-cap'] || '', '--file-cap', caller);
