...
import { AllSkippedError } from './errors.js';
...
/**
 * Escape control characters in an untrusted path before stderr
 * interpolation, so a forged directory or artifact name cannot inject fake
 * log lines or terminal-control sequences (e.g. `\x1b[2J` clear-screen).
 *
 * @param {string} value
 * @returns {string}
 */
const encodeForStderr = (value) => {
  let out = '';
