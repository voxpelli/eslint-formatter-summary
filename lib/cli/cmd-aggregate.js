...
const run = async (argv, _importMeta, { parentName }) => {
  ...
  // peowly v1.3.3 does not ship a `type: 'number'` flag — it's a planned
  // Meow-extension currently commented out in `lib/flag-types.d.ts`. Until
  // that lands (see UPSTREAM-peowly.md), CLIs declare numeric flags as
  // `type: 'string'` and validate here. Delete this helper when peowly
   ...
   */
  const parseNumericFlag = (raw, flagName) => {
    if (raw === '') return;
    const n = Number(raw);
    if (!Number.isFinite(n)) {
      process.stderr.write(`eslint-summary aggregate: ${flagName} must be numeric (got "${raw}")\n`);
      process.exit(2);
    }
    return n;
  };
  ...
  if (sortByRaw !== 'project' && sortByRaw !== 'severity') {
    process.stderr.write(`eslint-summary aggregate: --sort-by must be "project" or "severity" (got "${sortByRaw}")\n`);
    process.exit(2);
  }
  ...
  try {
    ...
    if (code === 'ENOENT' || code === 'ENOTDIR') {
      process.stderr.write(`eslint-summary aggregate: results directory not found: ${dir}\n`);
      process.exit(1);
    }
    throw err;
  }
