 ...
 */
export function prepareProjectResult (raw, { baseDir, project = '', eslintVersion = '' }) {
...
function prepareLintMessage (rawMsg) {
  if (
    ...
    // `line` is optional — fatal parse errors can emit no line. Messages
    // without one are still bucketed, with a `?` placeholder in the file
    // entry (matching the sibling tool's `msg.line ?? '?'` convention).
    !isOptionalKeyWithType(rawMsg, 'line', 'number')
  ) {
    return;
  }
  ...
  // `line` is optional at runtime, so the literal is only a partial
  // LintMessage — the `??` fallbacks below fill the gaps for classifyMessage.
  const msg = /** @satisfies {Partial<Linter.LintMessage>} */ ({
    ...rawMsg,
