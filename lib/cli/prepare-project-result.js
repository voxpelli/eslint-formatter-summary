 ...
 * the project root. The caller is expected to pass a canonicalized `baseDir`
 * (e.g. via `realpathSync.native`) to handle macOS `/tmp` → `/private/tmp`.
 * Paths that escape `baseDir` keep their `../` prefix and trigger a single
 * warning for debuggability.
 *
