  ...
  return rules
    .map((rule) => {
      ...
      const filesList = markdown
        ? `<ul>${[...relativeFilePaths].map((p) => `<li>${escapeHtml(p)}</li>`).join('')}</ul>`
        : '';

      return markdown
        ? `| ${errors} | ${warnings} | ${fixable} | <details><summary>${markdownLink(escapeHtml(ruleId), docsUrl).padEnd(maxRule)}</summary>${filesList}</details> |`
        : `${
