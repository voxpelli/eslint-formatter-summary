...
test('prepareProjectResult returns undefined when input is not an array', () => {
  assert.equal(prepareProjectResult(null, { cwd }), undefined);
  assert.equal(prepareProjectResult({}, { cwd }), undefined);
  assert.equal(prepareProjectResult('string', { cwd }), undefined);
});
...
test('prepareProjectResult returns undefined when zero errors and warnings', () => {
  ...
  assert.equal(prepareProjectResult(raw, { cwd }), undefined);
});
...
test('prepareProjectResult aggregates one rule across files with path:line entries', () => {
  ...
  const result = prepareProjectResult(raw, { cwd, project: 'acme/app' });
  assert.ok(result);
...
test('prepareProjectResult buckets parser errors into (parser error) with line entries', () => {
  ...
  const result = prepareProjectResult(raw, { cwd });
  assert.ok(result);
...
test('prepareProjectResult emits <unexpected path> for paths outside cwd', () => {
  const raw = [{
    filePath: '/other/a.js',
    errorCount: 1,
    warningCount: 0,
