...
import { renderFileSpan, renderProjectLabelWithVersion } from './build-file-anchor.js';
...
/**
 * The closing anchor of a project block — the truncator searches for this
 * byte sequence to find safe truncation points between blocks.
 *
 * @type {string}
 */
export const PROJECT_BLOCK_CLOSE = '\n</details>\n\n';

/**
 * The opening tag of a project block. The truncator searches for `\n` + this
 * tag to find where the first block starts (preserving the header prefix).
 *
 * @type {string}
 */
export const PROJECT_BLOCK_OPEN_TAG = '<details>';
...
export function renderProjectBlock (result, { fileCap = DEFAULT_FILE_CAP } = {}) {
  ...
  const label = renderProjectLabelWithVersion(result);
  ...
  const ruleEntries = Object.entries(result.rules)
    ...
    .toSorted((a, b) =>
      toCount(b.errors) - toCount(a.errors) ||
      toCount(b.warnings) - toCount(a.warnings) ||
