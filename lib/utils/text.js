 ...
 * @returns {boolean}
 */
export const isC0C1Control = (code) => code < 0x20 || (code >= 0x7F && code <= 0x9F);

/**
 * @param {number} code
 * @returns {boolean}
 */
export const isBidiOrZeroWidth = (code) => CONTROLS.has(code);
...
function stripControls (text) {
  ...
  for (const ch of text) {
    ...
    if (cp !== undefined && !isBidiOrZeroWidth(cp)) out += ch;
  }
  return out;
}
