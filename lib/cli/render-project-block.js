...
export default function renderProjectBlock (result, { fileCap = DEFAULT_FILE_CAP } = {}) {
  ...
  const rows = ruleEntries.map((rule) => {
    ...
    const fixCol = ruleFixable > 0 ? `${ruleFixable} :wrench:` : '-';
    return `| ${ruleErrors} | ${ruleWarnings} | ${fixCol} | <details><summary><code>${escapeHtml(sanitize(rule.id))}</code></summary>${fileBlock}</details> |`;
  }).join('\n');
