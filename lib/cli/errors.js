import { isBidiOrZeroWidth, isC0C1Control } from '../utils/text.js';
...
/**
 * Escape control characters in an untrusted path before stderr / error-message
 * interpolation, so a forged directory or artifact name cannot inject fake
 * log lines or terminal-control sequences (e.g. `\x1b[2J` clear-screen, or the
 * C1 single-shift CSI `\x9b`). Covers C0 controls, DEL, and C1 controls.
 *
 * @param {string} value
 * @returns {string}
 */
export const encodeForStderr = (value) => {
  let out = '';
  for (const ch of value) {
    const code = ch.codePointAt(0);
    out += code !== undefined && (isC0C1Control(code) || isBidiOrZeroWidth(code))
