...
/**
 * Parse a numeric CLI flag declared as `type: 'string'`. Empty input returns
 * undefined so callers can distinguish "flag unset" from "flag set to 0";
 * non-finite input writes an error to stderr and exits with code 2.
 *
...
export const parseNumericFlag = (raw, flagName, caller) => {
  ...
  if (!Number.isFinite(n)) {
    process.stderr.write(`${caller}: ${flagName} must be numeric (got "${raw}")\n`);
    process.exit(2);
  }
  return n;
};
