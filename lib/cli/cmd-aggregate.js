...
import {
  AllSkippedError, encodeForStderr, InputError, isErrorWithCode,
} from './errors.js';
...
const run = async (argv, _importMeta, { parentName }) => {
  const cli = peowly({
    ...
    options: {
      ...
      'eslint-versions': {
        type: 'string',
        'default': '',
        description: 'Comma-separated ESLint versions for the clean-run message (env: EFS_ESLINT_VERSIONS)',
      },
      'size-cap': {
        type: 'string',
        'default': '',
  ...
  // Only used for the clean-run message — per-project labels read the version
  // from each artifact's `eslintVersion` field instead.
  const eslintVersions = (flags['eslint-versions'] || EFS_ESLINT_VERSIONS || '')
    .split(',')
    .map((v) => v.trim())
    .filter(Boolean);

  if (sortBy !== 'project' && sortBy !== 'severity') {
    throw new InputError(`--sort-by must be "project" or "severity" (got "${encodeForStderr(sortBy)}")`);
  }
  ...
  try {
    ...
    if (isErrorWithCode(cause) && (cause.code === 'ENOENT' || cause.code === 'ENOTDIR')) {
      throw new InputError(`results directory not found: ${encodeForStderr(dir)}`, { cause });
    }
    if (cause instanceof AllSkippedError) {
      // 100% skip rate is a misconfiguration, not a clean run — exit non-zero
      // without reaching the "all N pass" banner below.
      throw new InputError(cause.message, { cause });
    }
  ...
  if (results.length === 0) {
    output = renderSuccess(projectCount, eslintVersions.length ? eslintVersions : undefined);
  } else {
